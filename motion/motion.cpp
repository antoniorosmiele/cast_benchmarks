...
int main(int argc, char **argv) {
  ...
  if(argc>4 || argc<3) {
    std::cout << "USAGE: " << argv[0] << " frame1.bmp frame2.bmp [output folder]" << std::endl<< std::endl;
    std::cout << "The output is the list of bounding boxes" << std::endl;
    std::cout << "Specify the last parameter to save output bmps" << std::endl << std::endl;
    return 1;
  }
  if(argc==4){
    output=true;
  ...
  if(output){
/*    writeImage((std::string(outputdir)+"/output0.bmp").c_str(), &originalImage);
    writeImage((std::string(outputdir)+"/output1.bmp").c_str(), &bgImage);
    grey2rgb(&outputImage2, &greyImage);
    writeImage((std::string(outputdir)+"/output2.bmp").c_str(), &outputImage2);
    grey2rgb(&outputImage2, &bgGreyImage);
    writeImage((std::string(outputdir)+"/output3.bmp").c_str(), &outputImage2);
    grey2rgb(&outputImage2, &motionImage);
    writeImage((std::string(outputdir)+"/output4.bmp").c_str(), &outputImage2);
    grey2rgb(&outputImage2, &erosionImage);
    writeImage((std::string(outputdir)+"/output5.bmp").c_str(), &outputImage2);
    grey2rgb(&outputImage2, &blobImage);
    writeImage((std::string(outputdir)+"/output6.bmp").c_str(), &outputImage2);*/
    overlap_features_filter(&originalImage, &blobImage, 254, &outputImage);
    writeImage((std::string(outputdir)+"/output7.bmp").c_str(), &outputImage);
  }
...
int** blob_detection(sImage* originalImage, sImage* blobImage, int* num_of_bbs, bool output) {
  ...
  for(r=0, count=1; r<blobImage->rows; r++){
    for (c=0; c<blobImage->cols; c++){
      ...
      if(originalImage->data[r*blobImage->cols+c] == FG) {
         if(r>0 && blobImage->data[(r-1)*blobImage->cols+(c)] > 0)
          value=blobImage->data[(r-1)*blobImage->cols+(c)];
        else if(r>0 && c>0 && blobImage->data[(r-1)*blobImage->cols+(c-1)] > 0)
          value=blobImage->data[(r-1)*blobImage->cols+(c-1)];
        else if (r>0 && c<blobImage->cols && blobImage->data[(r-1)*blobImage->cols+(c+1)] > 0)
          value=blobImage->data[(r-1)*blobImage->cols+(c+1)];
        else if (r>0 && c<(blobImage->cols)-2 && blobImage->data[(r-1)*blobImage->cols+(c+2)] > 0)
          value = blobImage->data[(r-1)*blobImage->cols+(c+2)];
        else if (r>0 && c<(blobImage->cols)-3 && blobImage->data[(r-1)*blobImage->cols+(c+3)] > 0)
          value = blobImage->data[(r-1)*blobImage->cols+(c+3)];
  ...
  if(output)
    for(r=0; r<blobImage->rows; r++)
      for (c=0; c<blobImage->cols; c++){
        /*this line can be used to color blobs. it causes a bug in the image
        if the result of the multiplication (and %256 due to the overflow on 
        char) is 0*/
        blobImage->data[r*blobImage->cols+c]*=10;
