/**
 * Original source code (then extended and modified) distributed by:
 *
...
#define RGB_IMG 0
#define YCBCR_IMG 1
#define SUBSAMPLEDCHROMA 2
#define DCT 3
#define DCT_QUANTIZED 4
...
typedef struct __jpeg_data
{
  ...
  // sub-sampled chroma
  unsigned char* cb_sub;
  unsigned char* cr_sub;
...
double cos_lookup[8][8]; //Lookup table to speed discrete cosine transf.
unsigned char byte_buffer; // See write_byte function
int bits_written;       // See write_byte function
...
{
  data->red = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char)*3);
  data->green = data->red + data->num_pixel;
  data->blue = data->red + data->num_pixel*2;
  //data->red = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));
  //data->green = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));
  //data->blue = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));

  data->y = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char)*3);
  data->cb = data->y + data->num_pixel;
  data->cr = data->y + data->num_pixel*2;
  //data->y = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));
  //data->cb = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));
  //data->cr = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char));

  data->cb_sub = (unsigned char*)malloc(data->num_pixel*sizeof(unsigned char)/2);
  data->cr_sub = data->cb_sub + data->num_pixel/4;
  //data->cb_sub = (unsigned char*)malloc(data->num_pixel/4*sizeof(unsigned char));
  //data->cr_sub = (unsigned char*)malloc(data->num_pixel/4*sizeof(unsigned char));

  data->dct_y = (double*)malloc(data->num_pixel*sizeof(double)*3/2);
  data->dct_cb = data->dct_y + data->num_pixel;
  data->dct_cr = data->dct_y + data->num_pixel*5/4;
  ...
  data->dct_y_quant = (char*)malloc(data->num_pixel*sizeof(char*)*3/2);
  data->dct_cb_quant = data->dct_y_quant + data->num_pixel;
  data->dct_cr_quant = data->dct_y_quant + data->num_pixel*5/4;
  //data->dct_y_quant = (char*)malloc(data->num_pixel*sizeof(char*));
  //data->dct_cb_quant = (char*)malloc(data->num_pixel/4*sizeof(char*));
  //data->dct_cr_quant = (char*)malloc(data->num_pixel/4*sizeof(char*));
...
int read_ppm(char* file_name, jpeg_data* data ) {
  char temp[256];
  unsigned char *image;
  int depth;
  ...
  {
    printf("Only pictures with dimensions which are multiples of 16 are supported!\n");
    return 1;
  }
  ...
  image = (unsigned char*) malloc(sizeof(unsigned char)* (data->width) * (data->height) * 3);

  if(image)
    fread(image, sizeof(unsigned char), (data->width) * (data->height) * 3, f_in);
  else{
  ...
  for(int i=0, j=0; i < (data->width) * (data->height) ; i++, j+=3){
    data->red[i] = image[j];
    data->green[i] = image[j+1];
    data->blue[i] = image[j+2];
  }
 ...
 */
int rgb_to_ycbcr(jpeg_data* data) // CACHE MISS QUI
{
 ...
 */
void subsample_chroma(jpeg_data* data) // CACHE MISS
{
 ...
 */
 void dct_block(int gap, unsigned char in[], double out[])
{
 ...
 */
void dct(int blocks_horiz, int blocks_vert, unsigned char in[], double out[])
{
  int h,v;
 ...
 */
void quantize(int num_pixel, double in[], char out[], int quantizer[])
{
  int i;
  for (i=0; i<num_pixel; i++)
  {
 ...
 */
void zigzag(int num_pixel, char dct[])
{
  int i,j;
  int tmp[64];
 ...
 * diff(i) = dc(i) - dc(i-1)
 */
void diff_dc(int num_pixel, char dct_quant[])
{
  int i;
  char last = dct_quant[0];
...
/*
 * map the value to its category
 * category-id           DC/AC-coeff delta
 * -----------------------------------------
 *  0                        0
 *  1                      -1,1
 *  2                   -3,-2,2,3
...
/*
 * determine frequencies (DC) - mapped to categories
 */
void calc_dc_freq(int num_pixel, char dct_quant[], int freq[])
{
  int i;
  for (i=0; i<num_pixel; i+=64)
    freq[huff_category(dct_quant[i])]++;
}

/*
 * determine frequencies (num_zeros + AC) - bit0-3=num_preceding_zeros, bit4-7=category-id
 */
void calc_ac_freq(int num_pixel, char dct_quant[], int freq[])
{
  int i;
  int num_zeros = 0;
  int last_nonzero;
  for (i=0; i<num_pixel; i++)
  {
    ...
    freq[ ((num_zeros<<4)&0xF0) | (huff_category(dct_quant[i])&0x0F) ]++;
    num_zeros = 0;
  }
}
...
{
  int category = huff_category(dc_val);
  int category_code = huff->sym_code[category];
  int category_size = huff->sym_code_len[category];
  write_bits(f, category_code, category_size);

  unsigned int id = abs(dc_val);
  if (dc_val < 0)
    id = ~id;
  write_bits(f, id, category);
}
...
void encode_ac_value(FILE* f, int ac_val, int num_zeros, huff_code* huff)
{
  ...
  int category = huff_category(ac_val);
  int v = ((num_zeros<<4)&0xF0) | (category&0x0F);
 ...
 */
void write_coefficients(FILE* f, int num_pixel, char dct_quant[], huff_code* huff_dc, huff_code* huff_ac)
{
  int num_zeros = 0;
  int last_nonzero;
  int i;
...
void write_dht_header(FILE* f, int code_len_freq[], int sym_sorted[], int tc_th)
{
  ...
  fputc(tc_th, f); // table category (0=DC, 1=AC) and table id (0=luma, 1=chroma)
...
//  free(data.dct_cb);
//  free(data.dct_cr);
  free(data.dct_y_quant);
//  free(data.dct_cb_quant);
//  free(data.dct_cr_quant);
